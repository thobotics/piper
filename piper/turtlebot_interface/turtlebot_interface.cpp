...
#include <turtlebot_interface.h>


namespace piper {

/* ************************************************************************** */
TurtlebotInterface::TurtlebotInterface(ros::NodeHandle nh)
{
  ...
  if (nh.hasParam("robot/arm_state_topic"))
  {
    ...
    arm_state_sub_ = nh.subscribe(arm_state_topic_, 1, &TurtlebotInterface::armStateCallback, this);
  ...
  if (problem_.robot.isMobileBase() && nh.hasParam("robot/base_state_topic"))
  {
    ...
    base_state_sub_ = nh.subscribe(base_state_topic_, 1, &TurtlebotInterface::baseStateCallback, this);
...
/* ************************************************************************** */
void TurtlebotInterface::execute()
{
  size_t exec_step;
  double coll_cost;
...
/* ************************************************************************** */
void TurtlebotInterface::armStateCallback(const sensor_msgs::JointState::ConstPtr& msg)
{
  size_t index;
...
/* ************************************************************************** */
void TurtlebotInterface::baseStateCallback(const geometry_msgs::Pose::ConstPtr& msg)
{
...
void mainCallback(const std_msgs::Bool::ConstPtr& msg)
{
  ...
  piper::TurtlebotInterface turtlebot(nh);
  turtlebot.execute();
  ROS_INFO("Done.");
  ros::shutdown();
}
...
{
  ros::init(argc, argv, "turtlebot_interface");
